/mob/living/verb/hide_under_tables()
	set name = "Hide under tables"
	set category = "IC"

	var/datum/component/table_fiend/table_fiend = GetComponent(/datum/component/table_fiend)
	if(table_fiend)
		qdel(table_fiend)
	...
	if(layer < TABLE_LAYER)
		to_chat(src, span_warning("You can't hide under tables."))
		return
